...
def p(now_keys, keys):
    return ", ".join([keys[b] for a, b in now_keys])


def find_min(now_keys, true_keys, keys, access):
    global minimum
    print(p(now_keys, keys), minimum, " "*16, sep=' ', end="\r", flush=True)
    if len(now_keys) == len(keys):
        time = 0
        for i in range(len(now_keys)-1):
            time += access[now_keys[i][1]][now_keys[i+1][1]][1]
        return time
    ...
    my_keys = [i for i in range(1, len(keys))]
    my_keys.sort(key=lambda x: access[now_keys[-2][0]][x][1])
    my_keys = [0] + my_keys
    my_keys_t = list(map(lambda x: keys[x], my_keys))

    for i in range(1, len(keys)):
        last[0] = i
        last[1] = my_keys[i]
        now_keys[-1][0] = i
        if (keys[last[1]] in true_keys or not access[now_keys[-2][0]][last[1]][0].issubset(true_keys)):
            continue
        else:
            true_keys.add(keys[last[1]])
            min_time = min(min_time, find_min(now_keys, true_keys, keys, access))
            true_keys.remove(keys[last[1]])
    now_keys.pop()
    if min_time < minimum:
        minimum = min_time
        print(p(now_keys, keys), minimum, " "*16, sep=' ', end="\r", flush=True)
...
print()
print(find_min([[0, 0]], set(["start"]), keys, access))
